...
func TestAccAivenProjectVPC_backwardCompat(t *testing.T) {
	resourceName := "aiven_project_vpc.bar"
	dataSourceName := "data.aiven_project_vpc.vpc"
	projectName := acc.ProjectName()
	cloudName := "google-europe-west3"
	networkCIDR := "192.168.2.0/24"
	config := testAccProjectVPCResourceConfig(projectName, cloudName, networkCIDR)

	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() { acc.TestAccPreCheck(t) },
		Steps: acc.BackwardCompatibilitySteps(t, acc.BackwardCompatConfig{
			TFConfig:           config,
			OldProviderVersion: "4.55.2",
			Checks: resource.ComposeTestCheckFunc(
				resource.TestCheckResourceAttr(resourceName, "project", projectName),
				resource.TestCheckResourceAttr(resourceName, "cloud_name", cloudName),
				resource.TestCheckResourceAttr(resourceName, "network_cidr", networkCIDR),
				resource.TestCheckResourceAttr(resourceName, "state", "ACTIVE"),
				...
				resource.TestCheckResourceAttr(dataSourceName, "network_cidr", networkCIDR),
...
func testAccProjectVPCResourceConfig(projectName, cloudName, networkCIDR string) string {
	return fmt.Sprintf(`
resource "aiven_project_vpc" "bar" {
  project      = %[1]q
  cloud_name   = %[2]q
...
data "aiven_project_vpc" "vpc" {
  project    = aiven_project_vpc.bar.project
  cloud_name = aiven_project_vpc.bar.cloud_name
}`, projectName, cloudName, networkCIDR)
}
