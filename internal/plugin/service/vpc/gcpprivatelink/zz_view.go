...
const typeName = "aiven_gcp_privatelink"

// idFields the ID attribute fields, i.e.:
// terraform import aiven_gcp_privatelink.foo PROJECT/SERVICE_NAME
...
func createView(ctx context.Context, client avngen.Client, d adapter.ResourceData) error {
	rsp, err := client.ServicePrivatelinkGoogleCreate(ctx, d.Get("project").(string), d.Get("service_name").(string))
	if err != nil {
		return err
	}
...
func readView(ctx context.Context, client avngen.Client, d adapter.ResourceData) error {
	rsp, err := client.ServicePrivatelinkGoogleGet(ctx, d.Get("project").(string), d.Get("service_name").(string))
	if err != nil {
		return err
	}
	return d.Flatten(rsp)
...
func deleteView(ctx context.Context, client avngen.Client, d adapter.ResourceData) error {
	_, err := client.ServicePrivatelinkGoogleDelete(ctx, d.Get("project").(string), d.Get("service_name").(string))
	return err
}
