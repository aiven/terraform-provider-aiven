...
type apiModel struct {
	...
	SubnetsBastion            *map[string]*string       `json:"aws_subnets_bastion,omitempty"`
	SubnetsWorkload           *map[string]*string       `json:"aws_subnets_workload,omitempty"`
...
func flattenData[R any](ctx context.Context, state *tfModel, rsp *R, modifiers ...util.MapModifier[R]) diag.Diagnostics {
	...
	if api.BucketNames != nil && (len(*api.BucketNames) > 0 || !state.BucketNames.IsNull()) {
		vBucketNames, diags := types.MapValueFrom(ctx, types.StringType, api.BucketNames)
		if diags.HasError() {
			return diags
		}
	...
	if api.SubnetsBastion != nil && (len(*api.SubnetsBastion) > 0 || !state.SubnetsBastion.IsNull()) {
		vSubnetsBastion, diags := types.MapValueFrom(ctx, types.StringType, api.SubnetsBastion)
		if diags.HasError() {
			return diags
		}
	...
	if api.SubnetsWorkload != nil && (len(*api.SubnetsWorkload) > 0 || !state.SubnetsWorkload.IsNull()) {
		vSubnetsWorkload, diags := types.MapValueFrom(ctx, types.StringType, api.SubnetsWorkload)
		if diags.HasError() {
			return diags
		}
