...
func (vw *view) Create(ctx context.Context, plan *tfModel) diag.Diagnostics {
	...
	diags := expandData(ctx, plan, nil, &req, legacyReqCastConfigLegacyFields)
	if diags.HasError() {
		return diags
	}
	...
	return util.MergeSlices(diags, legacyFakeRead(ctx, plan))
}
...
func (vw *view) Update(ctx context.Context, plan, state, config *tfModel) diag.Diagnostics {
	...
	diags = expandData(
		ctx, plan, state, &req,
		legacyReqCastConfigLegacyFields,
		legacyReqOwnerUserGroupID(state),
...
func (vw *view) Read(ctx context.Context, state *tfModel) diag.Diagnostics {
	...
	return util.MergeSlices(diags, flattenData(
		...
		legacyRspTerminationProtection,
		legacyRspCastConfigLegacyFields,
		legacyRspOwnerUserGroupID(state),
	))
}
...
func flattenRspConfigValues(state *tfModel) util.MapModifier[kafkatopic.ServiceKafkaTopicGetOut] {
	return func(rsp util.RawMap, in *kafkatopic.ServiceKafkaTopicGetOut) error {
		...
		// The Plugin Framework requires all Computed fields to have values.
		// Fills with zero values first to ensure all keys are present.
		// todo: remove in v5.0.0. Mark all fields optional, not computed+optional.
		config := legacyZeroConfig()
