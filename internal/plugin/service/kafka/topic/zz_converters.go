...
type tfModelConfig struct {
	...
	InklessEnable                   types.Bool    `tfsdk:"inkless_enable"`
...
func expandData[R any](ctx context.Context, plan, state *tfModel, req *R, modifiers ...util.MapModifier[apiModel]) diag.Diagnostics {
	api := new(apiModel)
	if !plan.Config.IsNull() || state != nil && !state.Config.IsNull() {
	...
	if !plan.TopicDescription.IsNull() || state != nil && !state.TopicDescription.IsNull() {
...
func flattenConfig(ctx context.Context, api *apiModelConfig) (*tfModelConfig, diag.Diagnostics) {
	...
	if api.InklessEnable != nil {
		state.InklessEnable = types.BoolPointerValue(api.InklessEnable)
	}
