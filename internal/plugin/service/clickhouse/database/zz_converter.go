...
func (tf *tfModel) SetID(vProject string, vServiceName string, vName string) {
	tf.Project = types.StringValue(vProject)
	tf.ServiceName = types.StringValue(vServiceName)
	tf.Name = types.StringValue(vName)
	tf.ID = types.StringValue(filepath.Join(vProject, vServiceName, vName))
}
...
func flattenData[R any](ctx context.Context, state *tfModel, rsp *R, modifiers ...util.MapModifier[tfModel]) diag.Diagnostics {
	...
	if state.ID.ValueString() != "" {
		...
		if state.Name.ValueString() == "" {
			state.Name = types.StringValue(parts[2])
		}
	}
	state.SetID(state.Project.ValueString(), state.ServiceName.ValueString(), state.Name.ValueString())
	return nil
}
