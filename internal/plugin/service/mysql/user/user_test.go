...
func TestAccAivenMySQLUser_basic(t *testing.T) {
	...
	// Create and Update views use different types of "state".
	// So we need to make sure that creating a user without a password works as expected.
			...
			Steps: []resource.TestStep{
				{
					...
					Config: testAccMySQLUserWithoutPassword(projectName, serviceName, userName),
					Check: resource.ComposeTestCheckFunc(
						...
						// Control user without password gets autogenerated password
						func(state *terraform.State) error {
							r, ok := state.RootModule().Resources[resourceName]
							assert.True(t, ok, "control resource not found in state")
							assert.True(t, strings.HasPrefix(r.Primary.Attributes["password"], "AVN"))
							return nil
						},
					),
				},
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						resource.TestCheckResourceAttr("data.aiven_mysql_user.user", "username", userName),
						resource.TestCheckResourceAttr("data.aiven_mysql_user.user", "project", projectName),
						resource.TestCheckResourceAttr("data.aiven_mysql_user.user", "service_name", serviceName),
					),
				},
				{
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						func(state *terraform.State) error {
							...
							require.NoError(t, err, "retrieving mysql user after recreation")
...
func testAccMySQLUserWithoutPassword(projectName, serviceName, userName string) string {
	return fmt.Sprintf(`
resource "aiven_mysql_user" "foo" {
  project      = %[1]q
  service_name = %[2]q
