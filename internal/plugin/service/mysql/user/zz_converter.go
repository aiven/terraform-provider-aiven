...
const typeName = "aiven_mysql_user"
...
func expandData[R any](ctx context.Context, plan, state *tfModel, req *R, modifiers ...util.MapModifier[tfModel]) diag.Diagnostics {
	...
	if !plan.Authentication.IsNull() && !plan.Authentication.IsUnknown() {
		vAuthentication := plan.Authentication.ValueString()
		api.Authentication = &vAuthentication
	}
...
func flattenData[R any](ctx context.Context, state *tfModel, rsp *R, modifiers ...util.MapModifier[tfModel]) diag.Diagnostics {
	...
	if api.Authentication != nil || state.Authentication.IsUnknown() {
		state.Authentication = util.StringPointerValue(api.Authentication)
	}
