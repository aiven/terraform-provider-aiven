...
func readView(ctx context.Context, client avngen.Client, state *tfModel) diag.Diagnostics {
	...
	func() {
		rsp, err := client.UserGroupMemberList(ctx, state.OrganizationID.ValueString(), state.UserGroupID.ValueString())
		if err != nil {
			diags.Append(errmsg.FromError("UserGroupMemberList Error", err))
			return
		}
		diags.Append(flattenData(ctx, state, &map[string]any{"members": rsp})...)
	}()
	return diags
}
