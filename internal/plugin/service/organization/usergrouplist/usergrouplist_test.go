...
func TestAccAivenOrganizationUserGroupListDataSource(t *testing.T) {
	const (
		dataSourceName = "data.aiven_organization_user_group_list.ds"
...
`, acc.OrganizationName(), groupName, description),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(dataSourceName, "user_groups.0.user_group_id"),
					resource.TestCheckResourceAttrSet(dataSourceName, "user_groups.0.create_time"),
					resource.TestCheckResourceAttrSet(dataSourceName, "user_groups.0.update_time"),
				),
			},
		},
	})
