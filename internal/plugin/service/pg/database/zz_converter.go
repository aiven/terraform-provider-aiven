...
func expandData[R any](ctx context.Context, plan, state *tfModel, req *R, modifiers ...util.MapModifier[tfModel]) diag.Diagnostics {
	...
	if !plan.LcCollate.IsNull() && !plan.LcCollate.IsUnknown() {
		vLcCollate := plan.LcCollate.ValueString()
		api.LcCollate = &vLcCollate
	}
	if !plan.LcCtype.IsNull() && !plan.LcCtype.IsUnknown() {
		vLcCtype := plan.LcCtype.ValueString()
		api.LcCtype = &vLcCtype
	}
...
func flattenData[R any](ctx context.Context, state *tfModel, rsp *R, modifiers ...util.MapModifier[tfModel]) diag.Diagnostics {
	...
	if api.LcCollate != nil || state.LcCollate.IsUnknown() {
		state.LcCollate = util.StringPointerValue(api.LcCollate)
	}
	if api.LcCtype != nil || state.LcCtype.IsUnknown() {
		state.LcCtype = util.StringPointerValue(api.LcCtype)
	}
