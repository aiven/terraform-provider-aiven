...
func TestAccAivenPGDatabase(t *testing.T) {
  ...
  lc_ctype   = ""`)
	...
	t.Run("base test", func(t *testing.T) {
		...
		configLcEmpty := testAccPGDatabaseWithDatasource(projectName, serviceName, dbName, `  lc_collate = ""
  lc_ctype   = ""`)
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					// Checks SDKv2 parity: empty lc_* must not trigger replacement.
					Config:   configLcEmpty,
					PlanOnly: true,
					Check: resource.ComposeTestCheckFunc(
