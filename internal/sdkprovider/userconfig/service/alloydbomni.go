...
func alloydbomniUserConfig() *schema.Schema {
	return &schema.Schema{
		Description:      "Alloydbomni user configurable settings",
		...
		Elem: &schema.Resource{Schema: map[string]*schema.Schema{
			...
			"pg": {
				...
				Elem: &schema.Resource{Schema: map[string]*schema.Schema{
					...
					"log_line_prefix": {
						Description:  "Enum: `'pid=%p,user=%u,db=%d,app=%a,client=%h '`, `'pid=%p,user=%u,db=%d,app=%a,client=%h,txid=%x,qid=%Q '`, `'%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '`, `'%m [%p] %q[user=%u,db=%d,app=%a] '`. Choose from one of the available log formats.",
						Optional:     true,
						Type:         schema.TypeString,
						ValidateFunc: validation.StringInSlice([]string{"'pid=%p,user=%u,db=%d,app=%a,client=%h '", "'pid=%p,user=%u,db=%d,app=%a,client=%h,txid=%x,qid=%Q '", "'%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '", "'%m [%p] %q[user=%u,db=%d,app=%a] '"}, false),
					},
			...
			"pgbouncer": {
				...
				Elem: &schema.Resource{Schema: map[string]*schema.Schema{
					...
					"autodb_pool_mode": {
						Description:  "Enum: `transaction`, `session`, `statement`. PGBouncer pool mode. Default: `transaction`.",
						Optional:     true,
						Type:         schema.TypeString,
						ValidateFunc: validation.StringInSlice([]string{"transaction", "session", "statement"}, false),
					},
					"autodb_pool_size": {
