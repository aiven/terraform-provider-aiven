...
var aivenGovernanceKafkaAccessSchema = map[string]*schema.Schema{
	...
	"username": {
		...
		Description:  userconfig.Desc("The name that will be used for the new service user assigned to the access. If not provided, name is automatically generated").ForceNew().MaxLen(54).Build(),
	},
	"acls": {
		Type:        schema.TypeSet,
		Required:    true,
...
var aivenGovernanceAccessSchema = map[string]*schema.Schema{
	...
	"access_type": {
		...
		Description:  userconfig.Desc("The type of access. For example KAFKA.").ForceNew().PossibleValuesString(governance.AccessTypeChoices()...).Build(),
	},
	"access_data": {
		Type:             schema.TypeList,
		Description:      userconfig.Desc("The data defined by access_type.").ForceNew().Build(),
		Required:         true,
		ForceNew:         true,
...
func ResourceGovernanceAccess() *schema.Resource {
	return &schema.Resource{
		Description:   userconfig.Desc(`Creates and manages governance access for an organization. Governance access provide convenient management and access to service resources and is part of Aiven Kafka Governance`).Build(),
