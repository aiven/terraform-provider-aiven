...
func newKafkaTopicAvailabilityWaiter(
	ctx context.Context,
	client *aiven.Client,
	project string,
	serviceName string,
	topicName string,
) (*kafkaTopicAvailabilityWaiter, error) {
...
func (w *kafkaTopicAvailabilityWaiter) refresh() error {
	...
	v2Topics, err := w.Client.KafkaTopics.V2List(w.Context, w.Project, w.ServiceName, queue)
	if err != nil {
		...
		if aiven.IsNotFound(err) {
			...
			list, err := w.Client.KafkaTopics.List(w.Context, w.Project, w.ServiceName)
			if err != nil {
