...
func readKafkaQuotaWithRetry(ctx context.Context, d *schema.ResourceData, client avngen.Client) error {
	...
	return retry.RetryContext(ctx, 5*time.Second, func() *retry.RetryError {
		...
		// check if any configured value doesn't match what the API returned
		cMiss := expectedConsumerRate != nil && expectedConsumerRate != actualConsumerRate
		pMiss := expectedProducerRate != nil && expectedProducerRate != actualProducerRate
		rMiss := expectedRequestPct != nil && expectedRequestPct != actualRequestPct

		if cMiss || pMiss || rMiss {
