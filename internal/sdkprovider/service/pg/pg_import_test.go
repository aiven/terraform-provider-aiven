...
func TestAccAivenPG_import(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccPGImportResource(projectName, rName),
			...
			},
			{
				...
				ImportStateCheck: func(s []*terraform.InstanceState) error {
					...
					assert.Equal("pgimporttest@aiven.io", attributes["tech_emails.0.email"])
...
func testAccPGImportResource(project string, name string) string {
	return fmt.Sprintf(`
...
resource "aiven_pg" "read_replica" {
  project                 = data.aiven_project.foo.project
  cloud_name              = "google-europe-west1"
  plan                    = "startup-4"
  ...
  service_integrations {
    source_service_name = aiven_pg.main.service_name
