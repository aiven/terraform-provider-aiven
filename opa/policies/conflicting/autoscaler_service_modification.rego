...
services_being_modified contains service_name if {
	...
	# extract service name from either before or after state
...
deny contains msg if {
	...
	startswith(service_config.type, "aiven_")
	not service_config.type == "aiven_service_integration"
	not service_config.type == "aiven_service_integration_endpoint"
	...
	msg := sprintf(
		concat("", [
			...
			"This can cause inconsistent Terraform plans due to disk space management conflicts. ",
			"Please separate these operations: ",
			"1) First, remove the autoscaler integration alone, or ",
			"2) Add explicit 'additional_disk_space' configuration before removing the autoscaler.",
		]),
