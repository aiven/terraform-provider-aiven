...
The `elasticsearch_user_config` block supports:
...
* `elasticsearch` - (Optional) Elasticsearch settings. Structure is documented below.
...
* `index_patterns` - (Optional) Glob pattern and number of indexes matching that pattern to 
    be kept. Structure is documented below.

* `kibana` - (Optional) Kibana settings. Structure is documented below.
...
* `index_template` - (Optional) Template settings for all new indexes.
    Structure is documented below.
...
The `index_patterns` block supports:
...
The `private_access` block supports:
...
The `index_template` block supports:
