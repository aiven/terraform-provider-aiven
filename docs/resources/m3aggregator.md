...
- `m3aggregator_user_config` (Block List, Max: 1, Deprecated) M3aggregator user configurable settings. **Warning:** There's no way to reset advanced configuration options to default. Options that you add cannot be removed later (see [below for nested schema](#nestedblock--m3aggregator_user_config))
...
- `ip_filter_object` (Block Set, Max: 8000) Allow incoming connections from CIDR address block, e.g. `10.20.0.0/16` (see [below for nested schema](#nestedblock--m3aggregator_user_config--ip_filter_object))
