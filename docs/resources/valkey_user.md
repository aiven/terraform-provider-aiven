...
- `password_wo` (String, Sensitive) The password of the service user (write-only, not stored in state). Must be used with `password_wo_version`. Must be 8-256 characters.
...
- `valkey_acl_keys` (List of String) Key access rules. Entries are defined as standard glob patterns. The field is required with `valkey_acl_categories` and `valkey_acl_keys`.
