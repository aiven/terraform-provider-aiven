...
- `config` (Block List) Kafka topic configuration. (see [below for nested schema](#nestedblock--config))
...
- `inkless_enable` (Boolean) (DEPRECATED) Use diskless.enable instead. Indicates whether diskless should be enabled. This is only available for BYOC services with Diskless feature enabled. This configuration will be removed in future versions.
