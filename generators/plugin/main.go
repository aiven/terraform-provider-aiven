...
func genDefinition(doc *OpenAPIDoc, defPath string) error {
	...
	files := map[resDatType]*SchemaMeta{
	...
	for _, entity := range sortedKeys(files) {
		...
		file := newFile(
			pkgName,
			fmtImport(isResource, schemaPackageFmt),
			fmtImport(isResource, planmodifierPackageFmt),
			fmtImport(isResource, timeoutsPackageFmt),
		)
...
func newFile(pkgName string, extraImports ...string) *jen.File {
	...
	// Adds imports for planmodifiers and validators.
	// Go-imports will remove unused imports
	for k := range typingMapping() {
		imports = append(imports, getTypedPlanmodifier(k), getTypedValidator(k))
	}
