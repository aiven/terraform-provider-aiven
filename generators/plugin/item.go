...
func (o Operations) AppearsInID(operationID OperationID, handler OperationType, source AppearsIn) AppearsIn {
	...
	operationTypes := make([]OperationType, 0) // CreateHandler, ReadHandler, etc.
	for i, op := range o {
		if op.ID == operationID && op.Type == handler {
			operationIndex = i
		}

		operationTypes = append(operationTypes, op.Type)
	}
	sources := listSources()                            // PathParameter, RequestBody, ResponseBody
	generic := len(sources) + len(listOperationTypes()) // Reserved bits for generic bits: sources and handlers

	// OperationID might appear in multiple operations (e.g., create and update)
	...
	return operationToHandler()[operationTypes[operationIndex]] | source | (1 << bitOffset)
}
